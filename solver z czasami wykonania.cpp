...
int main() {
    ...
    // Ładowanie danych z plików
    cout << "Trwa ladowanie wierzcholkow z pliku ..." << endl;
    loadVertices(g, "struktury.txt");
    cout << "Trwa ladowanie drog z pliku i tworzenie listy sasiedztwa ..." << endl;
    loadEdges(g, "drogi.txt");

    g.splitVertices();
    vector<Polygon> cwiartki = loadPolygonsFromFile("cwiartki.txt");
    cout << "Trwa dodawanie boosta do pol na podstawie cwiartek ..." << endl;
    output1 << "Trwa dodawanie boosta do pol na podstawie cwiartek ..." << endl;
    start = clock();
    applyBoostToFields(g.vertices, cwiartki);
    stop = clock();
    exec_time = static_cast<double>(stop - start) / CLOCKS_PER_SEC;
    cout << "Czas dodania boostow do pol uprawnych: " << exec_time << " s" << endl;
    output1 << "Czas dodania boostow do pol uprawnych: " << exec_time << " s" << endl;
    cout << "---------------------------------------------------------" << endl;
    output1 << "---------------------------------------------------------" << endl;
    
    cout << "Trwa tworzenie sieci przeplywowej ..." << endl;
    output1 << "Trwa tworzenie sieci przeplywowej ..." << endl;
    start = clock();
    ...
    cout << "Czas tworzenia sieci przeplywowej: " << exec_time << " s" << endl;
    output1 << "Czas tworzenia sieci przeplywowej: " << exec_time << " s" << endl;
    cout << "---------------------------------------------------------" << endl;
    output1 << "---------------------------------------------------------" << endl;

    cout << "Trwa obliczanie przeplywu jeczmienia (podsiec 1) ... " << endl;
    output1 << "Trwa obliczanie przeplywu jeczmienia (podsiec 1) ... " << endl;
    start = clock();
    int maxJeczmien = edmondsKarp(g, "SOURCE", "SOURCEBREWERY"); // maksymalna ilosc jęczmienia, którą można dostarczyć do browarów
    stop = clock();
    exec_time = static_cast<double>(stop - start) / CLOCKS_PER_SEC;
    cout << "EDMONTS-KARP: Czas obliczenia przeplywu jeczmienia: " << exec_time << " s" << endl;
    output1 << "EDMONTS-KARP: Czas obliczenia przeplywu jeczmienia: " << exec_time << " s" << endl;
    ...
    for (int i = 0; i < g.vertices.size(); i++) {      
        output2 << g.indexToId[i] << " (" << g.vertices[i].x << "," << g.vertices[i].y << ") -> ";
        for (const auto& edge : g.adjList[i]) {          
            output2 << g.indexToId[edge.to] << "(cap:" << edge.capacity << ") ";
        }       
        output2 << endl;
    }

    output2 << "=== Wierzcholki ===" << endl;
    for (const auto& v : g.vertices) {
        output2 << v.global_id << " (" << v.x << "," << v.y << "), typ: " << v.type << ", spec: " << v.spec << endl;
    }
    ...
    for (int i = 0; i < g.adjList.size(); ++i) {
        ...
        for (const Edge& e : g.adjList[i]) {
            ...
            output2 << fromId << " -> " << toId << " | przepustowosc: " << e.capacity << ", flow: " << e.flow << ", cost: " << e.cost << endl;
        }
    }

    //cout << "--------------------------------------------------------------------" << endl;
    cout << "Maksymalny przeplyw jeczmienia: " << maxJeczmien << endl;
    output1 << "Maksymalny przeplyw jeczmienia: " << maxJeczmien << endl;
    cout << "---------------------------------------------------------" << endl;
    output1 << "---------------------------------------------------------" << endl;

    cout << "Trwa obliczanie przeplywu piwa (podsiec 2) ... " << endl;
    output1 << "Trwa obliczanie przeplywu piwa (podsiec 2) ... " << endl;
    start = clock();
    int maxFlow = edmondsKarp(g, "SOURCEBREWERY", "SINK");
    stop = clock();
    exec_time = static_cast<double>(stop - start) / CLOCKS_PER_SEC;
    cout << "EDMONTS-KARP: Czas obliczenia przeplywu piwa: " << exec_time << " s" << endl;
    output1 << "EDMONTS-KARP: Czas obliczenia przeplywu piwa: " << exec_time << " s" << endl;

    cout << "Maksymalny przeplyw piwa: " << maxFlow << endl;
    output1 << "Maksymalny przeplyw piwa: " << maxFlow << endl;
    cout << "---------------------------------------------------------" << endl;
    output1 << "---------------------------------------------------------" << endl;
    ...
    cout << "Trwa obliczanie przeplywu piwa i obliczenia kosztu naprawy drog (podsiec 2) ... " << endl;
    output1 << "Trwa obliczanie przeplywu piwa i obliczenia kosztu naprawy drog (podsiec 2) ... " << endl;
    start = clock();
    int achievedFlow = minCostFlow(g, "SOURCE", "SINK", maxFlow, totalRepairCost);
    stop = clock();
    exec_time = static_cast<double>(stop - start) / CLOCKS_PER_SEC;
    cout << "MIN COST MAX FLOW: Czas obliczenia przeplywu piwa i obliczenia kosztu naprawy drog: " << exec_time << " s" << endl;
    output1 << "MIN COST MAX FLOW: Czas obliczenia przeplywu piwa i obliczenia kosztu naprawy drog: " << exec_time << " s" << endl;
    cout << "Uzyskana przepustowosc po naprawie drog: " << achievedFlow << endl;
    output1 << "Uzyskana przepustowosc po naprawie drog: " << achievedFlow << endl;
    cout << "Koszt naprawy drog potrzebnych do osiagniecia przeplywu: " << totalRepairCost << endl;
    output1 << "Koszt naprawy drog potrzebnych do osiagniecia przeplywu: " << totalRepairCost << endl;
    cout << "---------------------------------------------------------" << endl;
    output1 << "---------------------------------------------------------" << endl;
    output1.close();
    return 0;
